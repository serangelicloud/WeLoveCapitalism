...
public class ChestShopDatabase {
    ...
    public static void deleteChestShop(ChestShop shop) throws SQLException {
        ...
        statement.setString(1, shop.getOwner());
    ...
    public static HashSet<ChestShop> parseChestShops() throws SQLException, IOException, ClassNotFoundException {
        ...
        while(set.next()){
            ...
            if(Objects.requireNonNull(Bukkit.getWorld("world")).getBlockAt(chest_loc).getType() != Material.CHEST){
                continue;
            }

            if(Objects.requireNonNull(Bukkit.getWorld("world")).getBlockAt(sign_loc).getType() != Material.OAK_WALL_SIGN){
                continue;
            }

            Chest chest = (Chest) Objects.requireNonNull(Bukkit.getWorld("world")).getBlockAt(chest_loc).getState();

            Sign sign = (Sign) Objects.requireNonNull(Bukkit.getWorld("world")).getBlockAt(sign_loc).getState();
