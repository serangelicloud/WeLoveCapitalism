...
public class TownyUpkeepListener implements Listener {
    ...
    void onNewDay(NewDayEvent event) {
        for(Nation nation : TownyAPI.getInstance().getNations()){
            /* Don't check this for fallen nations since they're gone anyway */
            ...
            if(after_balance < 0){
                ...
                account.setBalance(0, "WLC trade law upkeep");

                for(Trade trade : TradeManager.Trades){
            ...
            account.setBalance(after_balance, "WLC trade law upkeep");
