...
public class ChestShopManager {
    ...
    public static void buyChestShop(ChestShop shop, Player player, int amount) throws SQLException {
        ...
        Trade trade;
        try {
            trade = new Trade(shop.getID(), ChestShopDatabase.serializeItemStack(item), amount, (int)received_gold, UUID.fromString(shop.getOwner()), player.getUniqueId(), shop.getTownUUID(), shop.getNationUUID(),  player_town_uuid, player_nation_uuid, System.currentTimeMillis());
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
